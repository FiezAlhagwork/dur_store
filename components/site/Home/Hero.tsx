...
export default function Hero() {
  ...
  return (
    <>
      ...
      <section
        className="relative h-screen w-full overflow-hidden bg-second"
        id="hero"
      ...
      >
        ...
        */}
        ...
        <div className="relative z-20 flex h-[calc(100vh-80px)] items-center px-6 lg:px-32">
          <motion.div
            className={[
              // `mx-auto` explicitly centers this box up through tablet —
              // once max-w-lg actually caps the width (viewports wider
              // than ~512px), it would otherwise sit flush at the start
              // edge instead of in the middle like it does on phones,
              // where the box just happens to fill the available width.
              // `lg:mx-0` hands that back to the desktop layout below,
              // unchanged.
              "w-full max-w-lg text-center mx-auto lg:mx-0",
          ...
          >
            <motion.h1
              ...
              className={`${
                isArabic
                  ? // md: bumps tablet only — lg: restores the original
                    // fluid clamp so desktop is untouched.
                    "text-[clamp(2.3rem,7vw,4.2rem)] md:text-[4.5rem] lg:text-[clamp(2.3rem,7vw,4.2rem)] font-bold"
                  : "text-[clamp(2rem,5vw,2.6rem)] md:text-[3rem] lg:text-[clamp(2rem,5vw,2.6rem)] font-extrabold"
            ...
            <motion.div
              ...
              className="mt-8 flex flex-col items-center justify-center gap-4 font-sans sm:flex-row lg:justify-start"
            >
              <Button
